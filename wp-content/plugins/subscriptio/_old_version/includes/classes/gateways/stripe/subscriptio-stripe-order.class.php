    ...
    public function capture($order_id)
    {
        ...
        if (RightPress_WC_Legacy::order_get_payment_method($order) == 'subscriptio_stripe' && RightPress_WC_Legacy::order_get_meta($order, '_subscriptio_stripe_charge_captured', true) === 'no') {
            ...
            if (!is_object($response)) {
                $order->add_order_note(esc_html__('Failed capturing previously authorized Stripe payment.', 'subscriptio-stripe') . ' ' . $response);
                return;
            }

            // Received error from Stripe?
            if (!empty($response->error)) {
                $order->add_order_note(esc_html__('Failed capturing previously authorized Stripe payment.', 'subscriptio-stripe') . ' ' . $response->error->message);
                return;
            }
