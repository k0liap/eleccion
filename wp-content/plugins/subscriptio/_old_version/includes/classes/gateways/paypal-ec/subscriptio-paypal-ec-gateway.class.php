...
class Subscriptio_PayPal_EC_Gateway extends WC_Payment_Gateway
{
    ...
    public function get_notices()
    {
        ...
        if ($this->sandbox == 'no' && empty($this->api_username) || empty($this->api_password) || empty($this->api_signature)) {
            $notices['secret'] = esc_html__('PayPal Express Checkout payment gateway requires all API credentials to be set.', 'subscriptio-paypal-ec');
        }
    ...
    public function show_admin_notices()
    {
        foreach ($this->notices as $notice) {
            echo '<div class="error"><p>' . esc_html__($notice, 'subscriptio-paypal-ec') . '</p></div>';
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        if (!$order) {
            ...
            RightPress_Helper::wc_add_notice(esc_html__('Order not found. We have not charged you for this order. Please try again.', 'subscriptio-paypal-ec'), 'error');
            return;
        }
        ...
        // Check cart
            ...
            RightPress_Helper::wc_add_notice(esc_html__('The cart is empty - we have not charged you for this order.', 'subscriptio-paypal-ec'), 'error');
            return;
        }
    ...
    public function do_express_checkout_payment()
    {
        ...
        if (empty($token) || (empty($payer_id) && RightPress_WC_Legacy::order_get_total($order) != 0) || empty($order_id)) {
            ...
            // Add error
            RightPress_Helper::wc_add_notice(esc_html__('Unfortunately, your session for this payment has expired. Please try again.', 'subscriptio-paypal-ec'), 'error');
        ...
        // Prevent double payments
            ...
            // Add error
            RightPress_Helper::wc_add_notice(esc_html__('Operation cancelled - current token was already used to make a payment.', 'subscriptio-paypal-ec'), 'error');
    ...
    public function handle_trial($order, $token)
    {
        ...
        $this->log_add(esc_html__('Starting process of handling trial subscription from order #', 'subscriptio-paypal-ec') . RightPress_WC_Legacy::order_get_id($order));
	...
	$nvp_response = $this->send_curl_request($nvp_request);
        ...
        if ($result_message == 'Success' || $result_message == 'SuccessWithWarning') {
            $order->add_order_note(sprintf(esc_html__('%s of PayPal charge %s refunded.', 'subscriptio-paypal-ec'), Subscriptio::get_formatted_price($amount), $transaction_id));
