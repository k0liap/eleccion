...
class XStore_Advanced_Sticky_Header_Update {
	/**
	 * Plugin slug.
	 ...
	 * @since    1.0.0
	 */
	private $plugin_slug = 'xstore-advanced-sticky-header';
	...
	public function xstore_pre_set_site_transient_update_plugins($transient): object {
		...
		if ( version_compare( XStore_Advanced_Sticky_Header_Version, $latest_version, '<' ) ) {
			$plugin = new \stdClass();
			$plugin->slug = $this->plugin_slug;
			$plugin->plugin = $this->plugin_slug.'/'.$this->plugin_slug.'.php';
			$plugin->new_version = $latest_version;
			$plugin->url = XStore_Advanced_Sticky_Header_CHANGELOG;
			$plugin->package = $this->get_file_url();
			$plugin->tested = '6.1.1';
			$plugin->icons = Array(
				'2x' =>esc_url( XStore_Advanced_Sticky_Header_URL . '/images/256x256.png' ),
				'1x' =>esc_url( XStore_Advanced_Sticky_Header_URL . '/images/128x128.png' )
			);
...
new XStore_Advanced_Sticky_Header_Update();